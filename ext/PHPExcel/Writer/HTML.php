...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
	...
	/* 20160112 */
	private $dis_thead = false; //是否显示 thead
	public function setDisThead($b=false){
		$this->dis_thead = $b;
	}
	private $exclude_col = ''; //排除列显示  string 如 1,2,3
	public function setExcludeCol($cols=''){
		if($cols!='') $this->exclude_col = ','.$cols.',';
	}
	private $limit_col = 0; //列显示限制数  int
	public function setLimitCol($col=0){
		if($col>0) $this->limit_col = $col-1; //下标
	}
	/* 20160112 end */
     ...
     * @throws PHPExcel_Writer_Exception
     */
    public function generateSheetData() //20160111 generateSheetData()
    {
        // PHPExcel object known?
        if (is_null($this->phpExcel)) {
            throw new PHPExcel_Writer_Exception('Internal PHPExcel object not set to an instance of an object.');
        ...
        foreach ($sheets as $sheet) {
            ...
            if($this->limit_col!=0 && $dimension[1][0]>$this->limit_col) $dimension[1][0]=$this->limit_col; //超出指定列数
            ...
            $theadStart = $theadEnd = $this->dis_thead ? 1 : 0; // default: no <thead>    no </thead>
			$tbodyStart = $theadStart==0 ? $rowMin : $rowMin+1; //2016011 $tbodyStart = $rowMin;
            ...
            while ($row++ < $rowMax) {
                ...
                if (!isset($this->isSpannedRow[$sheet->getParent()->getIndex($sheet)][$row])) {
                    ...
                    while ($column++ < $dimension[1][0]) {
						if($this->exclude_col!='' && strpos($this->exclude_col, ','.($column+1).',')!==false){
                            continue;
                        }
                        // Cell exists?
    ...
    private function extendRowsForChartsAndImages(PHPExcel_Worksheet $pSheet, $row)
    {
        ...
        $html = '';
        $colMax++;
        while ($row < $rowMax) { //20160111 while ($row <= $rowMax) {
            $html .= '<tr>';
    ...
    public function buildCSS($generateSurroundingHTML = true)
    {
        ...
        foreach ($sheets as $sheet) {
			...
			if($this->limit_col!=0 && $highestColumnIndex>$this->limit_col) $highestColumnIndex=$this->limit_col; //超出指定列数
            $column = -1;
            while ($column++ < $highestColumnIndex) {
                ...
                $css['table.sheet' . $sheetIndex . ' col.col' . $column]['width'] = 'auto'; //42pt
            }
            ...
            foreach ($sheet->getColumnDimensions() as $columnDimension) {
                if (($width = PHPExcel_Shared_Drawing::cellDimensionToPixels($columnDimension->getWidth(), $this->defaultFont)) >= 0) {
                    ...
                    if($column>$highestColumnIndex) break;
    ...
    private function generateTableHeader($pSheet)
    {
       ...
       // $html .= $this->setMargins($pSheet);
        ...
        while ($i++ < $highestColumnIndex) {
            if (!$this->isPdf) {
				if($this->exclude_col!='' && strpos($this->exclude_col, ','.($i+1).',')!==false){
                    continue;
                }
                if (!$this->useInlineCss) {
