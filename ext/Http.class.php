...
class Http
{
	...
	{	
		$code = self::doInit($url, $timeout, $header);
	...
	static public function doPost($url, $data=null, $timeout=5, $header='') 
	{
		$code = self::doInit($url, $timeout, $header);
		if(!$code) return false;
		switch($code)
		{
			case 1:return self::curlPost($url,$data,$timeout,$header);break;
			case 2:return self::socketPost($url,$data,$timeout,$header);break;
			case 3:return self::phpPost($url,$data,$timeout,$header);break;
		}
		return false;
	}
	//通过Send自定义方式发送数据 $header string|array
	static public function doSend($url, $type='GET', $data=null, $timeout=5, $header='') 
	{
		$code = self::doInit($url, $timeout, $header);
		if(!$code) return false;
		switch($code)
		{
			case 1:return self::curlSend($url,$type,$data,$timeout,$header);break;
			case 2:return self::socketSend($url,$type,$data,$timeout,$header);break;
			case 3:return self::phpSend($url,$type,$data,$timeout,$header);break;
		}
		return false;
	}
	static public function doInit(&$url, &$timeout, &$header){
	...
	{
		return self::curlSend($url, 'GET', null, $timeout, $header);
	}
	//通过curl post数据 支持post string
	static public function curlPost($url, $data=null, $timeout=5, $header='') 
	{
		return self::curlSend($url, 'POST', $data, $timeout, $header);
	}
	//通过curl 自定义发送请求 
	static public function curlSend($url, $type='GET', $data=null, $timeout=5, $header='') 
	{
		/*
		...
		*/
		...
		if( false === $result = curl_exec($ch)) 
			curl_errno($ch) && Log::trace('err:'. curl_error($ch)."\nurl:".$url.($data!==null?"\ndata:".$data:''));
	...
	//通过socket post数据
	static public function socketPost($url, $data=null, $timeout=5,$header='') 
	{
		return self::socketSend($url, 'POST', $data, $timeout, $header);
	}
	//通过socket 自定义发送请求 
	static public function socketSend($url, $type='GET', $data=null, $timeout=5,$header='') 
	{
		$header=empty($header)?self::defaultHeader():$header;
	...
	static public function phpPost($url, $data=null, $timeout=5, $header='') 
	{
		return self::phpSend($url, 'POST', $data, $timeout, $header);
	}
	//通过file_get_contents 函数自定义Send数据
	static public function phpSend($url, $type='GET', $data=null, $timeout=5, $header='') 
	{
		$header=empty($header)?self::defaultHeader():$header;
		$opt_http = 'http';
		if(substr($url,0,5)=='https'){ //ssl
	...
	static private function defaultHeader()
	{
		...
		//$header.="Host: \r\n";
		//$header.="Referer: \r\n";
		$header.="User-Agent: Mozilla/5.0 (Windows NT 5.2; rv:37.0) Gecko/20100101 Firefox/37.0";
		return $header;
	}
