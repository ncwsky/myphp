...
/*
	<?php
		...
		$encode = "Candidates selected using the works, will be part of a formal written notice, it relates to the ownership";// 被加密的字符串
...
class AES{
	...
	public static function encode($string, $key){
		$aes = new self(true);// 把加密后的字符串按十六进制进行存储
		//$aes = new AES(true,true);// 带有调试信息且加密字符串按十六进制存储
		$keys = $aes->makeKey($key);//128bit
