...
class ReplyAck
{
    ...
    public const RETRY_QUEUE = '_retry_queue';
    public const MSG_LIST = '_msg_list';
    public const MSG_LIST_HASH = '_msg_list_hash';
    public const MSG_QUEUE = '_msg_queue';
    public const MSG_QUEUE_HASH = '_msg_queue_hash';
    public const MSG_ID = '_msg_id';
    ...
    public static function init(string $redisName = null, string $prefix = 'RA', int $timeout = 5): void
    {
        ...
        if (SrvBase::$instance->server->worker_id == 0) { #仅在其中一个进程初始定时器，不要运行到多个进程中去，会造成数据重复处理
            #定时验证应答
            SrvBase::$instance->server->tick(self::$time * 1000, function (): void {
                $redis = self::redis();
                $page = 1;
                $size = 100;
                while (true) {
            ...
            SrvBase::$instance->server->tick(2 * 1000, function () use ($replyAckQueue): void {
                $replyAckQueue->run();
            });
    ...
    #队列 把重试的消息放到队列 #msg = [$message_id > $nextIdx > $data[time:data]]
    public static function queue($message_id, $msg): void
    {
    ...
    #发布确认 消除缓存的消息或队列
    public static function puback($message_id, $retry = false): void
    {
...
class ReplyAckQueue
{
    ...
    public function run()
    {
        ...
        try {
            ...
            \myphp\Log::WARN(sprintf('file:%s, line:%s, err:%s', $e->getFile(), $e->getLine(), $e->getMessage()));
            return false;
        }
        return true;
    }
