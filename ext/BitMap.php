...
class BitMap
{
    ...
    public $bitData;
    private $bitSize;
    public const NULL_BYTE = "\x00"; #空字符
    ...
    public static function load($data)
    {
        ...
        if (!$nRet) {
            throw new \Exception('解析数据[m,n]失败');
        }
     ...
     * @param $byte
     */
    protected function toIndexByte($pos, &$index, &$byte): void
    {
        $index = $pos >> 3; # 2^3=8;
        $byte = self::$posMap[$pos & 7]; #0x01<<($pos & 7); #
    }
    public function setBit($pos): void
    {
    ...
    public function delBit($pos): void
    {
        $this->toIndexByte($pos, $idx, $byte);
        if (($this->bitData[$idx] & $byte) === $byte) { #判断存在
            $this->bitData[$idx] = $this->bitData[$idx] ^ $byte;
