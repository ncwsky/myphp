...
class Image
{
    ...
    public static function code(int $w = 80, int $h = 36, int $fontsize = 18, int $len = 4, int $type = 0, $callable = null, string &$code = '')
    {
        ...
        $font = self::$fontPath ?: __DIR__ . '/../inc/ggbi.ttf';
        if ($type > 0) {
            switch ($type) {
                case 1:
                    $chars = '0123456789';
                ...
                case 5:
                    ...
                    $font = self::$fontPath ?: __DIR__ . '/../inc/fzxbsjw.ttf';
                    break;
            }
        }
        if ($font && substr($font, 0, 5) == 'phar:') {
            if (file_exists($font)) {
                $name = md5($font);
                $_font = sys_get_temp_dir() . '/_' . $name . '.ttf';
                if (!file_exists($_font)) {
                    \myphp\Log::write($_font, 'font');
                    // 1. 获取 PHAR 内字体的流内容
                    $fontContent = file_get_contents($font);
                    // 2. 将字体复制到临时文件
                    if (!file_put_contents($_font, $fontContent)) {
                        throw new \Exception('PHAR内字体容复制到临时文件失败');
                    }
                }
                $font = $_font;
            } else {
                throw new \Exception('请将字体文件放入PHAR包外');
            }
        }
