...
final class myphp
{
    ...
    public static function autoload($class_name): void
    {
        ...
        if ($len = strpos($class_name, '\\')) { //命名空间类加载
            ...
            if (isset(self::$namespaceMap[$namespace])) { //优先加载命名空间映射
                ...
                if (self::$namespaceMap[$namespace][0] == '/' || (IS_WIN && strpos(self::$namespaceMap[$namespace], ':')) || strpos(self::$namespaceMap[$namespace], 'phar://') === 0) { //绝对路径 | win | phar
    ...
    private static function _runCA()
    {
        ...
        if (false === $res) {
            return self::res()->withBody(Helper::outMsg('auth fail'));
        }
        // 请求缓存处理
        $res = self::reqCache();
        if ($res === null) {
    ...
    private static function _auth()
    {
        ...
        if (strpos(self::$cfg['auth_login_model'], ','.$c.',') !== false || strpos(self::$cfg['auth_login_action'], ','.$c.'/'.$a.',') !== false) {
            $res = $auth ? call_user_func([$auth, $auth_login]) : call_user_func($auth_login);
            if ($res instanceof Response) {
                return $res;
            }
            if (!$res) {
            ...
            //验证此模块中需要验证的动作
            if (self::$cfg['auth_login_M_A'] === '' || strpos(self::$cfg['auth_login_M_A'], ',' . $c . '/' . $a . ',') === false) {
                return null;
            }
        }
        //启动验证方法(类方法|静态方法或函数) null|res|false
        return $auth ? call_user_func([$auth, $auth_action]) : call_user_func($auth_action);
    }
    // app项目初始化
    private static function _initApp($path, $isCLI = IS_CLI): void
    {
        ...
        if (!is_file($runConfig)) {
            file_put_contents($runConfig, file_get_contents(__DIR__ . '/tpl/config.php'));
        }
        // 写入测试Action
    ...
    public static function cache()
    {
        ...
        return Cache::getInstance($type, self::$cfg['cache_option']);
    }
    ...
    //分解m c a
    public static function deMCA(string $mca, bool $resetGet = false): ?array
    {
        $mca = trim($mca, '/');
        if ($mca === '') {
            return null;
    ...
    */
    ...
    {
        if (!$uri) {
            return $url ? ROOT_DIR . $url : rtrim(self::env('APP'), '/');
        }

        $normal = substr($uri, 0, 1) == '!';
        if ($normal) { //普通url模式
            $uri = substr($uri, 1);
        }
        ...
        //url映射
        static $url_maps = null;
        if (is_array(self::$cfg['url_maps']) && !empty(self::$cfg['url_maps']) && $url_maps === null) {
            $url_maps = array_flip(self::$cfg['url_maps']); //交换 kv->vk
        }
        //分析mac及参数
        $query = '';
        if ($pos !== false) {// 参数处理
            ...
            parse_str($query, $get);
            $vars = is_array($vars) ? array_merge($get, $vars) : $get;
        } else {
            $mca = $uri;
        }
        ...
        $url = $url ? ROOT_DIR . $url : rtrim(self::env('APP'), '/');
