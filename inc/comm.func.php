...
function random($len = 6, $chars = '0')
{
    if ($chars == '0') {
        ...
        $chars = 'abcdefghijkmnpqrstuvwxyzABCDEFGHIJKLMNPRSTUVWXYZ0123456789';
    } elseif ($chars == '11') {
...
function my_hash($verify = false, $echo = true)
{
    ...
    if ($verify) {
        if ($my_hash && isset($_GET['my_hash']) && $my_hash == $_GET['my_hash']) {
            ...
            if ($echo) {
                return out_msg('0:[hash]数据验证失败', \myphp\Helper::getReferer());
            } else {
                return false;
            }
        }
...
function GetCode($w = 80, $h = 36, $fontsize = 18, $len = 4, $reurl = false, $number = false)
{
    ...
    if (!$reurl) {
        return '<img src="'. $codeurl .'" alt="验证码,看不清楚?请点击刷新验证码" style="cursor:pointer; vertical-align:middle;" onclick="this.src=\''. $codeurl .'\'+\'&t=\'+Math.random();return false;" />';
    } else {
        return $codeurl;
    }
}
...
function getargs($args_item = '', $gtype = 1)
{
    ...
    if (getMd5($args_val.my_hash()) != $key) {
        throw new \Exception('Invalid args!'); //参数有效验证 $key.'!='.$args_val.'-'.my_hash()
    }

    $para = ['args' => $args_val, 'key' => $key];
    $args_val = explode('|', $args_val);
    $args_item = explode('|', $args_item);
    if (count($args_val) != count($args_item)) {
        throw new \Exception('args num error!'); //参数个数验证
    }
...
function make_thumb($image, $thumb_wh = '240_180')
{
    ...
    if (strpos($imgType, strtolower($ext)) === false) {
        return false; //图片验证
    }
...
function set_config($config, $file = "/config.php", $allow_val = null)
{
    ...
    foreach ($config as $k => $v) {
        if (is_array($allow_val) && !in_array($k, $allow_val)) {
            continue;
        }
        $v = trim($v);
...
function cookie($name, $value = '', $option = null)
{
    ...
    if ('' === $value) {//获取cookie值
        ...
        if (!isset(myphp::res()->header['Set-Cookie'])) {
            myphp::res()->header['Set-Cookie'] = [];
        }
...
function num2ch($num, $mode = true, bool $sim = true): string
{
    ...
    if (!$num && $mode) {
        return str_replace(['元','整'], '', $mode === 'rmb' ? $cnVal : '零'.$cnVal);
    }
    //整数部分
    $str = strrev($num);
    $out = [];
