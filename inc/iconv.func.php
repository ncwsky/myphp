<?php
define('CODETABLEDIR', dirname(__FILE__).'/encoding/');
define('CHARSET', isset($GLOBALS['cfg']['charset']) ? strtolower($GLOBALS['cfg']['charset']) : 'utf-8');
/**
 * utf8转gbk
 * @param $utfstr
 */
...
function gbk_to_pinyin($txt) {
	if(CHARSET != 'gbk') {
		$txt = iconv(CHARSET,'GBK//IGNORE',$txt);
	}	
	$l = strlen($txt);
	$i = 0;
	$pyarr = array();
